...
猫をモデルにしたいときは、`猫`オブジェクトを作ります。そのプロパティは、`色`、`年齢`、`機嫌`（いい、悪い、眠い）、`飼い主`（人オブジェクトですね、捨て猫ならそのプロパティは空白）です。

`猫`のアクションは、`喉を鳴らす`、`引っ掻く`、`餌を食べる`（`キャットフード`などで、それはまた`味`というプロパティを持つ別のオブジェクトになるでしょう。）
...
達成したいことが分かったので、Djangoでモデルリングの開始です！
...
ここにユーザーやブログポストの情報を格納します。データを格納するのにSQLiteデータベースを使います。これはDjangoのデフォルトのデータベースアダプターで、今はこれで十分です。
...
- `Post`はモデルの名前で、他の名前をつけることもできます（が、特殊文字と空白は避けなければいけません）。クラス名は必ず大文字で始めます。
...
それにはまずフィールドのタイプを決めなければいけません。（テキスト？数字？日付？他のオブジェクトの関係は？つまりユーザーか？）
...
コードの細かいところまでは説明し出すと時間がかかるので、ここではしませんが、モデルのフィールドや上記以上の定義のし方について知りたい方は是非Djangoドキュメントを見てみて下さい。(https://docs.djangoproject.com/en/1.8/ref/models/fields/#field-types)

`def publish(self):`は何かと言うと、これこそが先程お話した公開メソッドそのものです。`def`は、これはファンクション（機能）/メソッドといいう意味です。

メソッドは頻繁に何かを`return`します。一つの例が`__str__`メソッド にあります。このシナリオでは、`__str__()`を呼ぶと、ポストの表題のテキスト(**string**)が返ってきます。

もしモデルがまだはっきりつかめないようだったら、気軽にコーチに聞いて下さい！特にオブジェクトとファンクションを同時に習ったときはとても複雑なのはよく分かってますから。でも前ほど魔法みたいじゃないといいですけど！

### データベースにモデル用の表を作る

最後のステップは新しいモデルをデータベースに追加することです。まず、（今作った）モデルの中で少し変更があったことをDjangoに知らせなければなりまんから、`python manage.py makemigrations blog`とタイプします。こんな感じです。
