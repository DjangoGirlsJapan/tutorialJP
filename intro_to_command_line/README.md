...
さて、__コマンドライン__ あるいは __コマンドライン　インターフェイス__と呼ばれるこの画面は、キーボードで入力したテキストで命令を出してコンピューターと直接対話するように、ファイルを見たり、変更したりするものです。このコマンドラインは、 *cmd*, *CLI*, *プロンプト*, *コンソール* or *ターミナル* と呼ばれることもあります。
